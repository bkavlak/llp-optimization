...
import os
import sys
from optimization_func_d import *
...
with concurrent.futures.ProcessPoolExecutor(max_workers=18) as executor:
    ...
    for scenario_name in scenario_list:
        ...
        # Listing files
        input_d_list = glob.glob(input_d_path + 'DoktarVariableInput_' + '*.csv')
        #input_d_list = glob.glob(input_d_path + 'DoktarVariableInput_' + str(city_id) + '*.csv')
        ...
        list(executor.map(optim_func, district_id_list, input_d_list))
